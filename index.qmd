...
mean_card_fl <- mean_card_fl %>%
  mutate(cfmean2010 = ifelse(is.na(cfmean2010), 233.375, cfmean2010)) %>%
  mutate(difference = year_day - cfmean2010)
...
cpal <- colorNumeric(palette = "Blues", domain = mean_card_fl$difference, na.color = "#080808")

rpal <- colorNumeric(palette = "Reds", domain = mean_ruby_throat$difference, na.color = "#080808")

leaflet(mean_card_fl) %>%
  addProviderTiles(provider = "Esri") %>%
  addCircleMarkers(~longitude, ~latitude, color = ~cpal(difference), radius = 1) %>%
