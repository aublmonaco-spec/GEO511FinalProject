...
cpal <- colorNumeric(palette = "mako", domain = mean_card_fl$difference, na.color = NA, reverse = FALSE)
...
leaflet(mean_ruby_throat) %>%
  addProviderTiles(provider = "Esri") %>%
  addCircleMarkers(~Longitude, ~Latitude, color = ~rpal(difference), radius = 1) %>%
